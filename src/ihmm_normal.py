...
@njit(fastmath=False, error_model="numpy")
def break_sticks_numba(Pi, Beta, mus, alpha0, gamma, u, params, max_extra=10):
    """
...
def sample(y: np.array, 
    ...
    for it in range(n_samples):
        ...
        # break sticks
        pi, betas, mus, K = break_sticks_numba(pi, betas, mus, alpha0, gamma, u, params)
        ...
        pi = sample_transitions(states, alpha0*betas)
        if verbose:
            print('Iter: ', it, ", n states: ", mus.size, ", alpha: ", alpha0, ", gamma: ", gamma)
        if it >= burn_in:
            state_means += states
    ...
    return state_means, np.array(y_preds)
...
if __name__ == "__main__":
    ...
    t1 = perf_counter()
    states, pred = sample(y, n_samples=20000, burn_in=4000,params=params, y_pred=0.1, verbose=True)
    t2 = perf_counter()
    print(f'Took {round(t2-t1, 2)}s')
    pred = np.mean(pred)
